...
//    }
    ...
    public function show($id)
    {
        ...
        for($i=0; $i< count($grades); $i++){
            $addedBy = (User::select('name', 'surname')->where('id', $grades[$i]->addedBy)->paginate(1))[0];
            $editBy = (User::select('name', 'surname')->where('id', $grades[$i]->editedBy)->paginate(1))[0];
            $grades[$i]->addedBy = "{$addedBy->name} {$addedBy->surname}";
            $grades[$i]->editedBy = "{$editBy->name} {$editBy->surname}";
    }
        $iducznia = $id;
